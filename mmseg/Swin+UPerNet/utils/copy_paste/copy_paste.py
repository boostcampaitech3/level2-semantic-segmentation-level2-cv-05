...
def main(args):
    # input path
    segclass = os.path.join(args.input_dir, 'annotations')
    ...
    tbar = tqdm.tqdm(masks_path, total=len(masks_path))
    for mask_path in tbar:
        ...
        save_colored_mask(mask, os.path.join(args.output_dir, 'annotations', mask_filename))
        cv2.imwrite(os.path.join(args.output_dir, 'images', img_filename), img)


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", default="/opt/ml/input/data/copy_paste", type=str,
                        help="input annotated directory")
    parser.add_argument("--output_dir", default="/opt/ml/input/copy_paste", type=str,
